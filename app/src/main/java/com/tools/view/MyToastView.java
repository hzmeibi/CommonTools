...
public class MyToastView extends Toast {
     ...
     * @description
     */
    private Toast makeText(int resId, String text, int duration) {
        Toast result = new Toast(mContext);
        // 获取LayoutInflater对象
        LayoutInflater inflater = (LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        // 由layout文件创建一个View对象
     ...
     * @param str
     */
    public void show(int drawableId, String str) {
        mToast = mMyToastView.makeText(drawableId, str, 100);
        mToast.show();
        mId = id;
        show(drawableId, id, 100);
    }
     ...
     * @param duration
     */
    public void show(int drawableId, String str, int duration) {
        if (mToast != null && !TextUtils.isEmpty(str)) {
            mToast.cancel();
        }
        mToast = mMyToastView.makeText(drawableId, str, duration);
        mToast.show();
        mId = id;
    }
}
