...
public class RealtimeBlurView extends View {
    ...
    private boolean touchClearAble = false;
    private float translateX = -200;
    private float translateY = -200;
    private int blurWidth = 50;
    ...
    public void setTouchClearAble(boolean touchClearAble) {
        this.touchClearAble = touchClearAble;
    }
    ...
    protected void drawBlurredBitmap(Canvas canvas, Bitmap blurredBitmap, int overlayColor) {
        if (blurredBitmap != null) {
            ...
            if (touchClearAble) {
//                blurredBitmap = createCircleImage(blurredBitmap, 50);//清晰圆圈
                blurredBitmap = toConformBitmap(blurredBitmap);//蒙版圆圈
            }
...
//        paint.setFlags(Paint.ANTI_ALIAS_FLAG);
         ...
         * 首先绘制圆形
         */
        canvas.drawCircle(translateX / 4, translateY / 4 - min, min / 2, paint);
        /**
         * 使用SRC_IN
         */
     ...
     * @return
     */
    private Bitmap toConformBitmap(Bitmap background) {
        if (background == null) {
            return null;
        }
        if (foreground == null || !foreground.isRecycled()) {
            foreground = ((BitmapDrawable) getResources().getDrawable(R.drawable.round100)).getBitmap();
            foreground = Bitmap.createScaledBitmap(foreground, blurWidth, blurWidth, true);
        }

        //create the new blank bitmap 创建一个新的和SRC长度宽度一样的位图
        Bitmap newbmp = Bitmap.createBitmap(background.getWidth(), background.getHeight(), Bitmap.Config.ARGB_8888);
        Canvas cv = new Canvas(newbmp);
        //draw bg into
        cv.drawBitmap(background, 0, 0, null);//在 0，0坐标开始画入bg
        Paint paint = new Paint();
        paint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.XOR));//去掉两张图相交部分,中空. 必须要用 XOR  用SRC_OUT抠出来是正方形
        //draw fg into
        cv.drawBitmap(foreground, translateX / 4 - blurWidth / 2, translateY / 4 - blurWidth, paint);//在 0，0坐标开始画入fg ，可以从任意位置画入
        //save all clip
